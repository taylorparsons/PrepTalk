...
## Scope
- In: newest-first transcript display, markdown rendering in transcript + score summary, PDF/TXT export (PDF default, TXT includes score+rubric+transcript), session list + load/resume ordered by `updated_at` with rehydrate-only behavior, question status rehydration.
