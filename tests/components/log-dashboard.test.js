...
describe('log dashboard', () => {
  ...
  it('updates metrics from log summary', async () => {
    ...
    expect(metricValue('Turn completion checks')).toBe('7');
  ...
  it('uses error_count when error_event_count is missing', async () => {
    window.__APP_CONFIG__ = { uiDevMode: true };
    const summary = {
      client_disconnects: 2,
      server_disconnects: 4,
      gemini_disconnects: 3,
    ...
    expect(metricValue('Errors')).toBe('1');
    expect(metricValue('Error sessions')).toBe('1');
  });
});
